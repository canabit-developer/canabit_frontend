            ...
            <v-row>
                ...
                <v-col cols="12" sm="8" md="6" class="-mt-3"> 
                    ...
                    <form @submit.prevent="changePassword()" ref="formResetPassword">
                        ...
                        <v-text-field  :type="isPasswordVisible ? 'text' : 'password'" :append-icon="isPasswordVisible ? `mdi-eye-off-outline` : `mdi-eye-outline`" @click:append="isPasswordVisible = !isPasswordVisible"  required class="w-full md:w-1/2 m-1" hint="* Password must contain at least 8 uppercase letters, lowercase letters and numbers." v-model="form.old_password" prepend-inner-icon="em em-lock" label="Old Password" dense outlined></v-text-field>
                        <v-text-field :type="isPasswordVisible ? 'text' : 'password'" :append-icon="isPasswordVisible ? `mdi-eye-off-outline` : `mdi-eye-outline`" @click:append="isPasswordVisible = !isPasswordVisible" required class="w-full md:w-1/2 m-1" hint="* Password must contain at least 8 uppercase letters, lowercase letters and numbers." v-model="form.password" prepend-inner-icon="em em-closed_lock_with_key" label="New Password" dense outlined></v-text-field>
                        <v-text-field :type="isPasswordVisible ? 'text' : 'password'" :append-icon="isPasswordVisible ? `mdi-eye-off-outline` : `mdi-eye-outline`" @click:append="isPasswordVisible = !isPasswordVisible" required class="w-full md:w-1/2 m-1" hint="* Password must contain at least 8 uppercase letters, lowercase letters and numbers." v-model="form.password_confirm" prepend-inner-icon="em em-closed_lock_with_key" label="New Password Confirm" dense outlined></v-text-field>
